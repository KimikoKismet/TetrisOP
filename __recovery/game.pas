...
var
  bg : TPicture;
begin
  Control.Picture.LoadFromFile(gameModeSelection);                  /// nalouduje nápovědu
  ...
  kostickyImages := TDictionary<TKostickaEnum, TImage>.Create;      /// vytvoření mapy
  image := TImage.Create(Self);

  image.Picture.LoadFromFile('obrazky/CervenaKosticka.png');        /// načtení červené kostičky
  kostickyImages.Add(TKostickaEnum.CERVENA,image);

  image.Picture.LoadFromFile('obrazky/ModraKosticka.png');          /// načtení modré kostičky
  kostickyImages.Add(TKostickaEnum.MODRA,image);

  image.Picture.LoadFromFile('obrazky/ZelenaKosticka.png');         /// načtení zelené kostičky
  kostickyImages.Add(TKostickaEnum.ZELENA,image);

  image.Picture.LoadFromFile('obrazky/OranzovaKosticka.png');       /// načtení oranžové kostičky
  kostickyImages.Add(TKostickaEnum.ORANZOVA,image);

  image.Picture.LoadFromFile('obrazky/FialovaKosticka.png');        /// načtení fialové kostičky
  kostickyImages.Add(TKostickaEnum.FIALOVA,image);

  gameInit(nasledujKosticka);   /// zahájí hru
end;

/// procedura, ktará se vykoná při každém tiku
...
begin
 ...
 { case status of
  ...
  aktualKosticka.setTvar(tmp); // možná nebude potřeba
  posun(smer, pole);
end;

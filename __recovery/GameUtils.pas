...
begin
  ...
  x := kostka.getX() + smer.getX(); //změna souřednic
  ...
  for radek := 0 to (Length(kostka.getTvar)-1) do begin
    for sloupec := 0 to (Length(kostka.getTvar[0])-1) do begin
      ...
      if (((sloupec + x) >= Length(zdroj[0])) or ((sloupec + x) < 0)) then begin    //kontrola jestli kostka nenarazila na levou nebo pravou stěnou
        status := VlozeniKostkyStatus.KOLIZE_SE_STENOU;
        GoTo loop;
      end;

      if ((radek + y) = Length(zdroj)) then begin     //kontrola jestli kostka nedopadla ke spodní hraně
...
// vynuluje hodnoty v daném řádku
procedure umazRadek(radek : Integer; hraciPole : TArray<TArray<TKosticka>>);
var
  sloupec : Integer;
begin
...
// posune zbylé kostky dolů, aby zaplnili prázdný řádek
function posunZbytekDolu(prazdnyRadek : Integer; hraciPole : TArray<TArray<TKosticka>>) : TArray<TArray<TKosticka>>;
var
  copy : TArray<TArray<TKosticka>>;
